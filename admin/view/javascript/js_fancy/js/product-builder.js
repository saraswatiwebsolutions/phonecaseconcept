jQuery(document).ready(function($) {
	...
	$('#fpd-add-image-element, #fpd-add-text-element, #fpd-add-curved-text-element, #fpd-add-upload-zone').click(function(evt) {
		...
		if(this.id == 'fpd-add-image-element' || this.id == 'fpd-add-upload-zone') {
			//enter title
			$('#fnt_title').val(prompt(fpd_product_builder_opts.enterTitlePrompt+':', ""));
				$('#fnt_type_upload').val(this.id);

			 if($('#fnt_title').val() == null) {
				return false;
			}
			else if($('#fnt_title').val().length == 0) {
				fpdMessage(fpd_product_builder_opts.enterTitlePrompt+'!', 'error');
				return false;
			}
	      $('#modal-image').remove();
			$.ajax({
				url: 'index.php?route=common/filemanager&token=' + getURLVar('token')+'&target=fnt_image_upload',
				dataType: 'html',
				beforeSend: function() {
	...
	$elementLists.on('click', '.fpd-change-image', function(evt) {
		...
		$this.parents('li:first').attr('data-target','fnt-change-image');
	...
	$(document).on('hidden.bs.modal', '#modal-image', function (event) {
		if ($('#fnt_image_upload').attr('value')) {
			if ($('[data-target="fnt-change-image"]').length) {
				$listItem = $(document).find($('[data-target="fnt-change-image"]')),
				element = _getElementById($listItem.attr('id'));
				...
				$('[data-target="fnt-change-image"]').attr('data-target','');
				$('#fnt_image_upload').attr('value','');
			} else {
				var imageParams = {index: stage.getObjects().length-1};
				var addUploadZone = $('#fnt_type_upload').val();
				var elementTitle = $('#fnt_title').val();
				if(addUploadZone == 'fpd-add-upload-zone') {
				...
				$('#fnt_image_upload').attr('value','');
				$('#fnt_type_upload').attr('value','');
				$('#fnt_title').attr('value','');
			}
		}
	});
	...
	//text alignment
	$('.fpd-text-originX').find('.button').click(function(evt) {
	...
	function _setFormFields(element) {
		...
		if(element.type == 'i-text' || element.type == 'curvedText') {
			...
			$('select[name="font"]').val(element.fontFamily);
			...
			if(paramsObject.originX == undefined) {
				paramsObject.originX = 'center';
			}
			$('input[name="textAlign"]').val(paramsObject.textAlign);
			$('input[name="originX"]').val(paramsObject.originX);
			$('.fpd-align-'+paramsObject.textAlign+'').addClass('active');
			$('.fpd-originX-'+paramsObject.originX+'').addClass('active');
		...
		if(paramsObject.uploadZone == '1') {
			...
			$parametersForm.find('[name="x"],[name="y"],[name="scale"],[name="price"],[name="colors"],[name="draggable"],[name="resizable"],[name="rotatable"]').parents('tr').show();
