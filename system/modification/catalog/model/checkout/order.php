	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if($product['design']){
				$stage_width = $this->config->get('config_designs_stage_width');
				$stage_height = $this->config->get('config_designs_stage_height');
				$stage = $this->getSettingParameters($product['product_id']);
				if($stage){
					$stage_width  = $stage['stage_width'];
					$stage_height = $stage['stage_height'];
				}
				$this->db->query("INSERT INTO " . DB_PREFIX . "fnt_order_product_design SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id. "', design = '" . $this->db->escape($product['design']['product']) . "', price = '" . (float)$product['price_product_design']. "', stage_width = '" . (int)$stage_width. "', stage_height = '" . (int)$stage_height. "'");}
