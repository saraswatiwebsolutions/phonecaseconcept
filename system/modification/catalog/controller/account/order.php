...
$product_design_info = $this->model_account_order->getProductOrderDesign($this->request->get['order_id'], $product['order_product_id']);
				if($product_design_info){
					$design = $this->url->link('product/fnt_product_design', 'order_id=' . $order_id . '&product_id=' . $product['product_id'] . '&order_product_id=' . $product['order_product_id']);
				} else {
					$design = '';
				}
				$data['products'][] = array(
		...
		if ($order_info) {
			...
			if ($order_product_info) {
				...
				if ($product_info) {
					...
					$product_design_info = $this->model_account_order->getProductOrderDesign($this->request->get['order_id'], $order_product_info['order_product_id']);
					if($product_design_info){
						$this->load->library('fnt_cart');
						$this->fnt_cart = new FntCart($this->registry);
						$this->fnt_cart->addDesign($order_product_info['product_id'], $order_product_info['quantity'], $option_data,0, base64_decode($product_design_info['design']), (float)$price = $product_design_info['price'],'');
					} else {
